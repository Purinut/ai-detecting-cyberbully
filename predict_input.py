# -*- coding: utf-8 -*-
"""
Created on Sun Dec 15 11:33:02 2019
...
'''
blank = normalize_corpus(textual_data[:len(textual_data)])
...
while(1):
    predict_data = []
    predict_vec = []
    data = input("Enter some sentences: ")
    predict_data.append(data)
    try:
        predict_normalize = normalize_corpus(predict_data)
    except: 
        print('error at normalize')
    try:
        for i in range(0,len(predict_normalize)):
            vector = average_vector(predict_normalize[i], model)
            predict_vec.append(vector)
    except:
        print('error at vectorize')
    try:
        prediction = clf.predict(predict_vec)
        if(prediction[0] == 1):
            print('This is cyberbully')
        else:
            print('not a bully')
    except:
        print('error at svm')
